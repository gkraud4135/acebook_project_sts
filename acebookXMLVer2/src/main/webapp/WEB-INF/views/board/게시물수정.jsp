...
<body>
<form onsubmit="return 게시물등록하다()" action="/modifyboard" enctype="multipart/form-data" method="post">
공개범위
내용<input type="text" name="contents" value="${board.contents}"></input><br>
...
<br>
   <c:if test="${not empty attach}">
      ...
      <c:forEach var="attach" items="${attach}" varStatus="status">
		...
		<button type="button" onclick="첨부파일삭제(${status.index},${attach.sn});">삭제</button>
		</li>
      </c:forEach>
      </ul>
   </c:if>
